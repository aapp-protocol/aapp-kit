...
- `aapp matrix` (`P-30`): Re-derives `state_matrix.md` from the `Status:` lines in `.plans/current/*.md`, so the matrix can no longer drift from the plans it describes. Preserves the human-owned Roadmap and per-row annotations, deletes orphan rows, and surfaces unrecognized statuses instead of silently miscategorizing them. `--check` audits without writing. Covered by `tests/matrix_test.sh`.
...
### Fixed
- Lifecycle matrix updates (`P-30`): `aapp draft`, `freeze`, `freeze-start` and `start` now re-derive `state_matrix.md` before committing instead of patching a row's emoji in place, so a plan's row moves to the correct section. Previously a frozen or started plan kept its old section and only changed colour.
- Next Action footer resolution (`P-30`): `aapp status` resolves the frozen and incubator plan suggestions from each blueprint's `Status:` line via the status registry. The previous `awk` reads anchored on numbered matrix headings and returned nothing once headings changed, silently disabling both branches of the footer.
- Plan lane bucketing (`P-30`): `aapp plan-status` reports blueprints whose status matches no registry entry under a distinct `❓ Unrecognized` heading instead of folding them into the Incubator, where a typo'd status was invisible.
- Distribution Asset Preservation, .sample Convention & CLI Discovery (`P-24`): Preserves `examples/` in `$SHARE_DIR/examples/` during `aapp install` with clean directory purging, establishes canonical in-place `*.sample` naming for reference plugins and hooks to prevent accidental execution, adds engine pattern filtering in `resolve_plugin_entrypoint` and `aapp` switchboard ignoring `.sample` files, introduces standard extension points catalog and available sample discovery in `aapp plugins` and `aapp hooks`, and validates drop-in zero-footprint isolation.
...
### Fixed
- Plan Header Status on Archival (`lib/cmd_plan.sh`, `tests/install_test.sh`): Updates blueprint header status line to `* **Status:** ✅ Done` and appends an archival entry to `## 📦 6. Change Log` when executing `aapp done`, preventing completed plans from remaining permanently stamped as `⚡ In Development` in `.plans/done/` (#77).
- Antigravity IDE Autocomplete Parity (`P-29`, `#70`): Sets `disable-model-invocation: false` across all retained universal skills in `templates/skills/`, restoring slash command autocomplete in modern AI IDEs (Antigravity IDE, Claude Code).
- Dynamic Context Briefing Footer & Bare Ingestion (`P-29`, `#72`): Replaces static `/aapp-digest` looping suggestion in `cmd_status.sh` with state-aware Next Action evaluator and provides 3-tier fallback in bare `aapp-digest` invocations.
