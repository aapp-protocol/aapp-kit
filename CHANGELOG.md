...
### Fixed
- Signature-Gated Self-Consumption & `--keep` Flag (`aapp`, `lib/cmd_install.sh`, `lib/cmd_init.sh`, `lib/cmd_help.sh`, `README.md`, `CHEATSHEET.md`, `tests/install_test.sh`): Replaces unconstrained `rm -rf` basename allowlist with content-based kit signature and clean working tree checks (`is_safe_to_consume_kit_dir`), preserving directories with extra files, non-kit contents, or uncommitted modifications, and adds `--keep` (`-k`) flag to both `install` and `init` (#51). Expanded install test suite to 54 passing tests.
