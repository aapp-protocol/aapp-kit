...
6. **Ambiguity & Diagnostics:**
   - If 0 matches: Output `❌ Plan '<query>' not found.` followed by a list of available blueprints.
   - If >1 matches: Output `⚠️ Ambiguous plan reference '<query>'. Multiple candidates match:` followed by candidate paths.

### C. Concurrency & Multi-Agent ID Allocation
In multi-agent environments (e.g. Claude Code + Antigravity working concurrently):
1. **Digest Allocation:** `/aapp-digest` scans the highest existing Plan ID across `.plans/current/` and `.plans/done/` and increments by 1.
2. **Re-sync Requirement:** Before drafting, agents must perform `git -C .plans pull` (or inspect active remote tracking) to detect peer-allocated IDs.
3. **Git Collision Safety:** If two agents draft concurrently with the same ID, git detects the conflicting filename at merge/rebase time, and `planning_health.sh` Pair 4 flags duplicate IDs before execution.

### D. Skill & Lifecycle Command Integration
Update the canonical skill instructions in `templates/skills/`:
1. **`/aapp-freeze <plan>`**:
   - Accepts `<plan>` as ID (`P-9`, `9`), slug (`guard-path`), or filename. Empty query refused.
...
2. **Pair 5: Target Files vs Guard Section 2 Self-Protection (`check_pair5_target_files_self_protection`)**:
   - Greps declared `### 📂 Target Files` in every active blueprint in `.plans/current/` against the self-protection patterns from `blast-radius-guard.sh` Section 2 (`.githooks/*`, `.agents/skills/aapp-*`, `.claude/settings*`, `.agents/claude/*`, `.cursor/rules/*`).
   - If any active plan declares a Section 2 file as a target: emits a blocking validation failure:
     `❌ [Pair 5 Violation] Plan '<plan>' declares Target File '<path>' which violates Guard Section 2 self-protection! Move it to Out of Bounds and edit 'templates/' instead.`
...
- [ ] `.claude/skills/aapp-*` -> Governed symlinked skill copies.
- [ ] `.githooks/*` -> Engine git hooks must never be edited directly (tamper protection).
- [ ] `.plans/done/*.md` -> Historical completed plan filenames are preserved untouched on disk.
