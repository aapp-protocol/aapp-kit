...
* **`ISSUE-014`**: Support an explicit `--keep` flag in `cmd_init.sh` and `cmd_install.sh` to preserve the `aapp-kit` directory if requested.
...
### 2.3 Upgrade & Install Flags (`ISSUE-013`, `ISSUE-014`, `ISSUE-018`)
...
* In `lib/cmd_init.sh` & `lib/cmd_install.sh`:
  - Support `--keep` argument in `aapp init --keep` and `aapp install --keep` to preserve kit directory.
...
- [ ] 2.1 Update `lib/cmd_init.sh` for unclosed marker safety, missing python3 warning, and `--keep` flag.
- [ ] 2.2 Update `lib/cmd_upgrade.sh` to hoist `AAPP_VERSION` and handle upgrade suppression cleanly.
- [ ] 2.3 Update `lib/cmd_install.sh` to handle `AAPP_IS_UPGRADE` and `--keep`.
...
- [ ] 3.3 Add regression test in `tests/install_test.sh` for `--keep` flag and missing `END` marker.
...
- [ ] `lib/cmd_init.sh` -> Unclosed marker protection, python3 warning, `--keep` flag support.
- [ ] `lib/cmd_upgrade.sh` -> Subshell version export fix and upgrade flag.
- [ ] `lib/cmd_install.sh` -> Support `--keep` flag and `AAPP_IS_UPGRADE`.
...
- [ ] `tests/install_test.sh` -> Test `--keep` flag and unclosed marker handling.
